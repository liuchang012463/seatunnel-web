...
@Component
public class HttpDatasourceConnectivityVerificationStrategy
    ...
    public boolean supports(DatasourceVerifyContext context) {
        return context != null
                && context.getDbType() == DbType.HTTP
                && !"SINK".equalsIgnoreCase(context.getRole());
    }
        ...
        if (vo.getItems() == null || vo.getItems().isEmpty()) {
            ...
            vo.addItem(success
                    ? ClientDatasourceVerifyItemVO.success(
                            "HTTP_HOCON_CONNECTIVITY", "HTTP Engine 连通性",
                            "SeaTunnel Http Source 测试任务执行成功",
                            "任务成功", "Engine 可访问健康检查端点")
                    : ClientDatasourceVerifyItemVO.fail(
                            "HTTP_HOCON_CONNECTIVITY", "HTTP Engine 连通性",
                            StringUtils.defaultIfBlank(vo.getErrorMessage(), vo.getMessage()),
                            "任务成功", "Engine 无法访问健康检查端点"));
        }
        return vo;
    }
}
