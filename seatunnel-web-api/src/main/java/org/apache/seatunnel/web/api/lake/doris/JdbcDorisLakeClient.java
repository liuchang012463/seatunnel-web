...
public class JdbcDorisLakeClient implements DorisLakeClient {
    ...
    @Override
    public List<DorisColumnMetadata> listCatalogColumns(
            String catalogName, String databaseName, String tableName) {
        String catalog = DorisIdentifier.validate(catalogName);
        String database = DorisIdentifier.validate(databaseName);
        String table = DorisIdentifier.validate(tableName);
        String sql = "SHOW COLUMNS FROM " + DorisIdentifier.quote(catalog) + '.'
                + DorisIdentifier.quote(database) + '.' + DorisIdentifier.quote(table);
        return withConnection("list catalog columns", connection -> {
            try (Statement statement = connection.createStatement()) {
                configure(statement);
                try (ResultSet result = statement.executeQuery(sql)) {
                    ResultSetMetaData metadata = result.getMetaData();
                    int nameIndex = columnIndex(metadata, "Field", "COLUMN_NAME", "NAME", 1);
                    int typeIndex = columnIndex(metadata, "Type", "COLUMN_TYPE", "DATA_TYPE", 2);
                    int nullableIndex = columnIndex(metadata, "Null", "IS_NULLABLE", "NULLABLE", 3);
                    List<DorisColumnMetadata> columns = new ArrayList<>();
                    int ordinal = 1;
                    while (result.next()) {
                        ...
                        if (name != null && !name.isBlank()) {
                            columns.add(new DorisColumnMetadata(
                                    name.trim(), ordinal++, type == null ? "" : type.trim(),
                                    "YES".equalsIgnoreCase(result.getString(nullableIndex)),
                                    null, null, null));
                        }
                    }
                    return List.copyOf(columns);
    ...
    @Override
    public List<String> listCatalogDatabases(String catalogName) {
        String catalog = DorisIdentifier.validate(catalogName);
        return queryNamesStatement(
                "list catalog databases",
                "SHOW DATABASES FROM " + DorisIdentifier.quote(catalog));
    }

    @Override
    public List<String> listCatalogTables(String catalogName, String databaseName) {
        String catalog = DorisIdentifier.validate(catalogName);
        String database = DorisIdentifier.validate(databaseName);
        return queryNamesStatement(
                "list catalog tables",
    ...
    private static int columnIndex(
            ResultSetMetaData metadata, String first, String second, String third, int fallback)
            throws SQLException {
        for (String label : List.of(first, second, third)) {
            for (int index = 1; index <= metadata.getColumnCount(); index++) {
                if (label.equalsIgnoreCase(metadata.getColumnLabel(index))
                        || label.equalsIgnoreCase(metadata.getColumnName(index))) {
                    return index;
                }
            }
        }
        return Math.min(fallback, metadata.getColumnCount());
    }
    ...
    private List<String> queryNamesStatement(String operation, String sql) {
        return withConnection(operation, connection -> {
            try (Statement statement = connection.createStatement()) {
                configure(statement);
                try (ResultSet result = statement.executeQuery(sql)) {
