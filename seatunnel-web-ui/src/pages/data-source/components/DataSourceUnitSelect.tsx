...
const DataSourceUnitSelect: React.FC<DataSourceUnitSelectProps> = ({
  ...
  return (
    <div className={`datasource-unit-select ${showBusinessSystem ? '' : 'datasource-unit-select--single'}`}>
      {showBusinessSystem ? (
      ...
      <div className={showBusinessSystem ? 'grid grid-cols-1 gap-4 md:grid-cols-2' : undefined}>
        ...
        {showBusinessSystem ? (
          ...
          <Form.Item name="businessSystemId" hidden>
            <Input type="hidden" />
          </Form.Item>
        )}
      </div>
