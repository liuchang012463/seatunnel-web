...
const ManagedTableWizard: React.FC = () => {
  ...
  const create = async () => {
    if (!preview?.planFingerprint) return;
    ...
    try {
      const response = await createManagedTable(preview.planFingerprint);
