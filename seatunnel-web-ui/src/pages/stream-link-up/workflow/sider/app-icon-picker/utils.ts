...
import { ImageFile } from "../emoji-picker"
...
export type useLocalUploaderProps = {
  disabled?: boolean
  limit?: number
...
const uploadFile = async (url: string, file: File, onProgress?: (percent: number) => void) => {
  const formData = new FormData();
  formData.append('file', file);
  ...
  return request(url, {
    ...
    // },
    // requestType: 'form',
    onProgress: onProgress ? (event: { percent: number }) => {
