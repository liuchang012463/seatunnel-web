...
import FileTypeBaseInfoSection from './components/BaseInfoSection';
import FileTypeClientLinkSection from './components/ClientLinkSection';
...
const FileDetailPage = () => {
    ...
    return (
      ...
      >
        ...
        <span style={{ fontSize: 13, color: "var(--st-color-text-muted)" }}>
          任务数据在从列表进入时缓存；请回到文件引接任务列表，从任务行「更多 → 查看详情」重新进入。
        </span>
  ...
  const nextText = isBaseStep ? '下一步：客户端链接配置' : '进入文件同步配置';

  const hintText = isBaseStep
    ? '先完成基础配置，再进入客户端链接配置'
