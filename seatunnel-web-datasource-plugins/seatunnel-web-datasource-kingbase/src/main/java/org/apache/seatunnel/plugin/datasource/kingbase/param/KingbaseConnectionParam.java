...
/**
 * Kingbase connection parameters.
 */
@Data
...
public class KingbaseConnectionParam extends BaseConnectionParam {
     ...
     * Database Port
     */
    @FormField(label = "端口号", defaultValue = "54321", required = true, order = 2, type = FieldType.NUMBER, placeholder = "Please enter the port")
    ...
    @FormField(
            label = "驱动Jar包",
            order = 6,
            defaultValue = "kingbase8-8.6.1.jar"
    )
    ...
    public String toString() {
        return "KingbaseConnectionParam{" +
                "user='" + user + '\'' +
                ", password='" + password + '\'' +
                ", database='" + database + '\'' +
